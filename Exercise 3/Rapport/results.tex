...
This is because of the way our data was collected as explained in $1.2$. The data collection was started when the gauge pressure was seen to fall to $1 \textup{ atm}$ and stopped close to $0 \textup{ atm}$.
Because the gauge was only given in increments of $0.1 \textup{ atm}$ we would normally say that the uncertainty is onlt $0.05 \text{ atm}$, but because we were measuring it as it was changing, we will asumme our visual messurement had an uncertainty of $\sigma_{p} = 0.1 \textup{ atm}$.
We did however fail to consider that we did not always end the data collection at exactly $p_{\textup{0}} = 1 \textup{ atm}$ as assumed before. 
...
This makes sense if we assume our uncertainty of $\sigma_{p} = 0.1 \text{ atm}$ also applies to $p_{\textup{0}}$.
